...
public class GameCompleteMenu : MonoBehaviour
{
    ...
    private int burgerBunLimit = 15; //Maximim number of burger buns available to fire at one time.
    ...
    public void InstantiateBurger()
    {
        ...
        OnBurgerBunDestroyed onBurgerBunDestroyed = bun.AddComponent<OnBurgerBunDestroyed>();
        onBurgerBunDestroyed.Init(() => RemoveBurgerBun(bun));
        AudioManager.iAudioManager.PlaySpawnBurger();

        bunsSpawned.Add(bun);
    }

    public void RemoveBurgerBun(GameObject bun)
    {
        bunsSpawned.Remove(bun);
    }
