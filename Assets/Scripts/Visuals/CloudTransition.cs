...
public class CloudTransition : MonoBehaviour
{
    ...
    private bool cloudTansitionScaledTime = false;
    float divisibleAudioTrigger = 3.3f; // When should the audio play? What number should the cloud index be divisible by to create sound. Try to keep this to .5f varaibles expmple, 1.5f, 2, 2.5f, 3,3.5f etc.
    ...
    public IEnumerator MenuTransition(Action startAction, Action middleAction, Action endAction, float alterTransitionDuration)
    {
        ...
        AudioManager.iAudioManager.PlayCloudPop();
        ...
        for (int cloudIndex = 1; cloudIndex < clouds.Length; cloudIndex++)
        {
            ...
            StartCoroutine(MovementLibrary.AnimationCurveLerp(clouds[cloudIndex].gameObject, duration, Vector3.zero, newScale, cloudTansitionScaledTime, MovementLibrary.ObjectLerpType.LocalScale, transitionCurve, (() => CloudPopAudio(cloudIndex)), null, null));
    ...
    void CloudPopAudio(int index)
    {
        if (index % divisibleAudioTrigger == 0 || index % Mathf.RoundToInt(divisibleAudioTrigger) == 0)
        {
            AudioManager.iAudioManager.PlayCloudPop();
        }
    }
