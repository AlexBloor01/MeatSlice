...
public class PlayerController : MonoBehaviour
{
    ...
    private const int difficultyVarient = 4; //The dividing factor that controls the Rounding Factor in RoundToPoint.
    private const float distanceChopForgivenessValue = 0.24f; //The dividing factor that controls the Rounding Factor in RoundToPoint.


    private bool isFoodSliced = true; //This stops spamming the slice button while loading next food or in menus.
    private float currentFoodYScale = 1f; //Scale of the current foods Y scale.
    ...
    public void InstantiateNewFood()
    {
        ...
        if (currentFood != null)
        {
            ...
            newFood.transform.localScale = VectorLibrary.ReplaceVector(previousFood.transform.localScale, currentFoodYScale, 1);
            currentFood = newFood;
            movement.StartMovement(previousFood.transform.position);
        }
        else
        {
            newFood.transform.localScale = VectorLibrary.ReplaceVector(newFood.transform.localScale, currentFoodYScale, 1);
            currentFood = newFood;
            startingPlatform.MoveToNext(false, currentFoodYScale);
    ...
    void NextGenerationStep()
    {
        ...
        {
            LoadNextFood();
            float difference = currentFoodYScale;
            if (nextFood.transform.localScale.y != currentFood.transform.localScale.y)
            {
                difference = (currentFood.transform.localScale.y * 0.5f) + (currentFoodYScale * 0.5f);
            }
    ...
    private void SetNewScale()
    {
        ...
        prevFoodPos = new Vector3(RoundToPoint(prevFoodPos.x, difficultyVarient), 0, RoundToPoint(prevFoodPos.z, difficultyVarient));

        float distance = Vector3.Distance(currentFoodPos, prevFoodPos);

        // AudioManager.iAudioManager.PlaySquelch();
        ...
        // Chop or no chop?
        if (distance <= distanceChopForgivenessValue)
        {
            if (!playerScored)
            {
                AudioManager.iAudioManager.PlayScorePoint();
                Score.iScore.AddScore(foodType);
                playerScored = true;
            }
            currentFood.transform.position = new Vector3(previousFood.transform.position.x, 0f, previousFood.transform.position.z);
            return;
        }
        else
        {
        ...
        Vector3 boxPosition = GetBoxPosition(currentFoodPos, prevFoodPos, distance, out float xDiff, out float zDiff);
        ...
        //Score point if not a game over chop.
        if (!playerScored)
        {
            Score.iScore.AddScore(foodType);
            playerScored = true;
        ...
        AudioManager.iAudioManager.PlayChop();
        ...
        CreateSlice(newScale, xDiff, zDiff, prevFoodPos, currentFoodPos);
    }
    #endregion

    #region Create Slice
    //Creates a slice of the current food and updates the visuals for the current food and slice.
    private void CreateSlice(Vector3 newScale, float xDiff, float zDiff, Vector3 prevFoodPos, Vector3 currentFoodPos)
    {
        //Get the opposite position of the temppivot, this is where the slice needs to appear and material change.
        Vector3 oppositePivotPosition = GetBoxPosition(prevFoodPos, currentFoodPos, 0, out _, out _);
    ...
    private Vector3 GetBoxPosition(Vector3 currentFoodPos, Vector3 prevFoodPos, float distance, out float xDiff, out float zDiff)
    {
